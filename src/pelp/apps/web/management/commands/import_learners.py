...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        lms = LMSCanvas()
        result = lms.update_course_learners(course)

        if result.valid:
            self.stdout.write(
                self.style.SUCCESS(f'Successfully imported {result.num_total} learners. Created: {result.num_created}, Assigned: {result.num_assigned}'))
        else:
            self.stdout.write(
                self.style.ERROR('Error importing learners. Exceptions:'))
