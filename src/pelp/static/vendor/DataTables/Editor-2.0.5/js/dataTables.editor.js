...
var __dtCellSelector = function (out, dt, identifier, allFields, idFn, forceFields) {
    ...
    cells.indexes().each(function (idx) {
        ...
        var data = row.data();
        var idSrc = idFn(data);
        var fields = forceFields || __dtFieldsFromIdx(dt, allFields, idx.column, cells.count() > 1);
...
var dataSource$1 = {
    ...
    fakeRow: function (insertPoint) {
        ...
        for (var i = 0, ien = dt.columns(':visible').count(); i < ien; i++) {
            ...
            var fields = __dtFieldsFromIdx(dt, this.s.fields, visIdx, true);
    ...
    edit: function (identifier, fields, data, store) {
        ...
        if (!__dtIsSsp(dt, this) || this.s.editOpts.drawType === 'none') {
            ...
            var rowId = dataSource$1.id.call(this, data);
            var row;
    ...
    prep: function (action, identifier, submit, json, store) {
        ...
        if (action === 'create') {
            store.rowIds = $.map(json.data, function (row) {
                return dataSource$1.id.call(_this, row);
            });
        }
        if (action === 'edit') {
...
function bubble(cells, fieldNames, show, opts) {
    ...
    this._edit(cells, editFields, 'bubble', opts, function () {
        ...
        if (opts.buttons) {
            table.append(_this.dom.buttons);
        }
...
function bubblePosition() {
    var wrapper = $('div.DTE_Bubble'), liner = $('div.DTE_Bubble_Liner'), nodes = this.s.bubbleNodes;
    ...
    var top = position.top, left = (position.left + position.right) / 2, width = liner.outerWidth(), visLeft = left - (width / 2), visRight = visLeft + width, docWidth = $(window).width(), padding = 15; this.classes.bubble;
    wrapper.css({
        top: top,
        left: left
    });
...
function dependent(parent, url, opts) {
    ...
    $(field.node()).on(opts.event + '.edep', function (e) {
        ...
        if (typeof url === 'function') {
            var o = url.call(_this, field.val(), data, update, e);
            if (o) {
...
function inlineCreate(insertPoint, opts) {
    ...
    if (this._tidy(function () { _this.inlineCreate(insertPoint, opts); })) {
        return this;
    }
    // Set the default for the fields
    $.each(this.s.fields, function (name, field) {
        field.multiReset();
        field.multiSet(0, field.def());
        field.set(field.def());
    });
    this.s.mode = 'main';
    this.s.action = "create";
...
function open() {
    ...
    this._nestedOpen(function () {
        _this._focus($.map(_this.s.order, function (name) {
            return _this.s.fields[name];
        }), _this.s.editOpts.focus);
...
function submit(successCallback, errorCallback, formatdata, hide) {
    var _this = this;
    var fields = this.s.fields, errorFields = [], errorReady = 0, sent = false;
    ...
    var send = function () {
        ...
        _this._event('initSubmit', [_this.s.action], function (result) {
            ...
            sent = true;
            _this._submit(successCallback, errorCallback, formatdata, hide);
        });
    };
...
function title(title) {
    ...
    if (typeof title === 'function') {
        title = title(this, new DataTable$4.Api(this.s.table));
    }
    header.html(title);
    return this;
}
...
function _inline(editFields, opts, closeCb) {
    ...
    for (var i = 0; i < elements.length; i++) {
        var el = elements[i];
        var node = el.node;
        elements[i].children = node.contents().detach();
    ...
    setTimeout(function () {
        ...
        $(document)
            ...
            var isIn = false;
            for (var i = 0; i < elements.length; i++) {
                if (elements[i].field._typeFn('owns', target) ||
                    $.inArray(elements[i].node[0], $(target).parents()[back]()) !== -1) {
                    isIn = true;
                }
            }
            if (!isIn) {
...
var datatable = $.extend(true, {}, baseFieldType, {
    ...
    create: function (conf) {
        ...
        if (conf.footer) {
            $('<tfoot>')
                ...
                ? $('<tr>').append($.map(conf.footer, function (str) {
                    return $('<th>').html(str);
                }))
                : conf.footer)
                .appendTo(table);
        ...
        var dt = table
            ...
            var container = $(api.table(undefined).container());
            // Select init
            DataTable$2.select.init(api);
            // Append side button controls
            side
                .append(container.find('div.dataTables_filter'))
                .append(container.find('div.dt-buttons'))
                .append(container.find('div.dataTables_info'));
        })
            .DataTable($.extend({
            buttons: [],
            columns: [
            ...
            conf.editor.on('submitComplete', function (e, json, data, action) {
                if (action === 'create') {
                    var _loop_1 = function (i) {
                        dt
                            .rows(function (idx, d) { return d === json.data[i]; })
                            .select();
                    };
    ...
    set: function (conf, val, localUpdate) {
        ...
        conf.dt.rows(function (idx, data, node) {
            return val.indexOf(valueFn(data)) !== -1;
        }).select();
    ...
    _jumpToFirst: function (conf) {
        ...
        var idx = conf.dt.row({ selected: true, order: 'applied' }).index();
        var page = 0;
...
var Field = /** @class */ (function () {
    ...
    Field.prototype.multiSet = function (id, val) {
        var that = this;
        var multiValues = this.s.multiValues;
        var multiIds = this.s.multiIds;
        if (val === undefined) {
        ...
        var set = function (idSrc, val) {
            ...
            multiValues[idSrc] = that._format(val, that.s.opts.setFormatter);
        };
